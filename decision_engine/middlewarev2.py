...
class DecisionProxyMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        try:
            ...
            # 1) Try to get service from cache first
            ...
            if not service_data:
                ...
                try:
                    ...
                    REQUESTS_TOTAL.labels(decision="block").inc()
                    log_request(ip=client_ip, path=path, method=request.method, size=len(body), score=0, decision="block", reason="Service not found")
            ...
            # 2) Get allowed services list from cache
            ...
            if service_data["id"] not in allowed_service_ids:
                REQUESTS_TOTAL.labels(decision="block").inc()
                log_request(ip=client_ip, path=path, method=request.method, size=len(body), score=0, decision="block", reason="Service access denied")
            ...
            log_request(ip=client_ip, path=path, method=request.method, size=len(body), score=0, decision="block", reason="invalid_max_services_config")
            logger.error(f"Invalid MAX_SERVICES value: {e}")
            return JsonResponse({
            ...
            log_request(ip=client_ip, path=path, method=request.method, size=len(body), score=0, decision="block", reason="service_lookup_error")
            return JsonResponse({
